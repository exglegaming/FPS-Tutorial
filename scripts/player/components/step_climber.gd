...
func handle_step_climbing() -> void:
    ...
    for i: int in player.get_slide_collision_count():
        ...
        if _is_vertical_surface(collision):
            ...
            if measured_height > MIN_STEP_HEIGHT && measured_height <= step_height && _is_valid_step_direction(collision):
                ...
                step_status = "Step Found! Height: %s" % str(measured_height)
            else:
                step_status = "Step too high: %s" % str(measured_height)
            break
...
func _is_vertical_surface(collision: KinematicCollision3D) -> bool:
    ...
    if abs(normal.y) <=  surface_threshold:
        step_status = "CollisionShape: Vertical Collision Found! %s" % str(normal)
        return false
...
func _check_collision_surface(collision: KinematicCollision3D) -> bool:
    ...
    if result and abs(result.normal.y) <= surface_threshold:
        step_status = "Raycast: Vertical Collision Found! %s" % str(result.normal)
        return true
...
func _get_player_feet_position() -> Vector3:
    var feet_pos: Vector3 = player.global_position
...
func _measure_step_height(collision: KinematicCollision3D) -> float:
    ...
    var player_feet: Vector3 = _get_player_feet_position()
    var player_head_y: float = player.global_position.y + (player.standing_collision.shape.height / 2)
...
func _is_valid_step_direction(collision: KinematicCollision3D) -> bool:
    ...
    var movement_direction: Vector3 = player.transform.basis * Vector3(input_dir.x, 0, input_dir.y)
