...
func calculate_view_offset(delta: float) -> void:
    ...
    var speed: float = Vector2(velocity.x, velocity.y).length()
    if speed > 0.1 && player.is_on_floor():
        _step_timer += delta * (speed / bob_frequency)
        _step_timer = fmod(_step_timer, 1.0)
    else:
        _step_timer = 0.0
    var bob_sin: float = sin(_step_timer * 2.0 * PI) * 0.5 # 0.5 reduces the magnitude of the sine wave, i.e. less movement
...
## Takes a value of 0.0 to 1.0 for screen shake strength and tweens that offset on both the horizontal and vertical offsets of the camera over a period of seconds
