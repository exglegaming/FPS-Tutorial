...
func draw_circle_crosshair() -> void:
    ...
    for arc in arc_segments:
        var start_angle: Variant = arc[0]
        var end_angle: Variant = arc[1]

        var points := []
        var angle_step: Variant = (end_angle - start_angle) / segments

        for i in range(segments + 1):
            var angle: Variant = start_angle + i * angle_step
            var point: Vector2 = Vector2(radius * cos(angle), radius * sin(angle))
            points.append(point)
